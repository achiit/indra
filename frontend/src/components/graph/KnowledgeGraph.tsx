...
import { DOMAIN_COLORS, EDGE_COLORS, getConfidenceColor } from '@/lib/utils'
...
export function KnowledgeGraph({ data, onNodeClick, highlightedNodeId }: KnowledgeGraphProps) {
  ...
  const { runData } = useMemo(() => {
    ...
    const graphNodes = data.nodes.map((n: any) => {
      ...
      return {
        ...n,
        label: applyFlag(lblStr.length < 25 ? lblStr : lblStr.substring(0, 25) + '...'),
    ...
    return { runData: { nodes: graphNodes, links: graphLinks } }
    ...
    <div ref={containerRef} className="w-full h-full relative bg-[#0A0A0F] rounded-xl overflow-hidden border border-[#2A2A3A]">
      {dimensions.width > 0 && (
        <ForceGraph2D
          ...
          graphData={runData}
          nodeLabel="label"
          nodeColor={n => (n as GraphNode).id === highlightedNodeId ? '#00ffff' : ((n as GraphNode).color || '#6B7280')}
          nodeRelSize={4}
          linkColor={l => (l as GraphLink).color || '#3f3f46'}
          linkWidth={l => ((l as GraphLink).confidence || 0.5) * 2}
          linkLabel={l => `${(l as any).source.id || (l as any).source} ➔ ${(l as any).label || (l as any).type} ➔ ${(l as any).target.id || (l as any).target}\nConfidence: ${((l as any).confidence * 100).toFixed(0)}%`}
          ...
          nodeCanvasObject={(node, ctx, globalScale) => {
            ...
            ctx.fillStyle = node.id === highlightedNodeId ? '#00ffff' : (node.color || '#6B7280')
            ctx.fill()

            // Draw text label always
            const label = (node as GraphNode).label
            const fontSize = Math.max(5, 10 / globalScale)
            ctx.font = `${fontSize}px Inter, sans-serif`
            ctx.textAlign = 'center'
            ctx.textBaseline = 'middle'
            ctx.fillStyle = node.id === highlightedNodeId ? '#00ffff' : 'rgba(255,255,255,0.9)'
      ...
      <div className="absolute top-4 right-4 bg-[#111118]/80 backdrop-blur-sm border border-[#2A2A3A] rounded-lg p-3 pointer-events-none">
        ...
        <div className="flex flex-col gap-1.5 text-xs">
          <div className="flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-emerald-400" /> High (&gt;0.7)</div>
          <div className="flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-amber-400" /> Med (0.4-0.7)</div>
          <div className="flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-orange-400" /> Low (0.15-0.4)</div>
          <div className="flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-zinc-500" /> Stale (&lt;0.15)</div>
        </div>
      </div>
    </div>
  )
